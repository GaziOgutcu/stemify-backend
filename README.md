...
4. Poll `/api/job/{job_id}` every 2-5 seconds with the bearer token. Make sure the frontend starts only one polling interval per job and clears it when `preview_status === "ready"` and `full_processing_status` is either `ready` or `failed`.
5. Show the 15-second vocal/instrumental preview as soon as `preview_status === "ready"` using `job.stem_urls` / `job.preview_urls`; these point to `/api/preview/...` URLs that can be assigned directly to `<audio src>` without custom headers. Show the Pay button after preview is ready.
...
9. If `full_processing_status === "processing"`, show “Preparing full downloads in the background…” and keep polling. When `payment=success`, call `GET /api/payment/verify?session_id=<session_id>&job_id=<job_id>` with the bearer token. The backend retrieves the Checkout Session from Stripe, verifies the session/job/user mapping, marks the existing job paid, and returns paid download URLs only after both Stripe says it is paid and `full_processing_status === "ready"`. Do not create or upload a second job after payment.
